...
def filterDataFrame(df, target_property: str, spec, operand='greater'):
    if checkInBounds(target_property, spec):
        if (target_property in range_properties and operand=='greater'):
            new_df = df[df[target_property]>spec]
        elif (target_property in range_properties and operand=='less'):
            new_df = df[df[target_property]<spec]
        elif target_property in set_properties:
            new_df = df[df[target_property].astype(str).str.contains(spec)]
        return new_df
    else:
